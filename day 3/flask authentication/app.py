...
def validate_student_data(name: str, email: str, age: str, course: str, student_id: int | None = None) -> list[str]:
    errors = []
    if not name.strip():
        errors.append("Student name is required.")
    if not email.strip() or not validate_email(email):
        errors.append("A valid email address is required.")
    if not age.strip() or not age.isdigit() or int(age) <= 0 or int(age) > 120:
        errors.append("Age must be a number between 1 and 120.")
    if not course.strip():
        errors.append("Course or stream is required.")
...
def dashboard():
    ...
    return render_template("dashboard.html", user=user, student_count=student_count)
...
def add_student():
    ...
    if request.method == "POST":
        ...
        errors = validate_student_data(form["name"], form["email"], form["age"], form["course"])
        if errors:
            for error in errors:
                flash(error, "danger")
        ...
        cursor.execute(
            ...
            (form["name"], form["email"], int(form["age"]), form["course"]),
        )
        db.commit()
...
def edit_student(student_id):
    ...
    if request.method == "POST":
        ...
        errors = validate_student_data(form["name"], form["email"], form["age"], form["course"], student_id)
        if errors:
            for error in errors:
                flash(error, "danger")
            return render_template(
                "student_form.html",
                form=form,
                form_action=url_for("edit_student", student_id=student_id),
                title="Edit Student",
        ...
        cursor.execute(
            ...
            (form["name"], form["email"], int(form["age"]), form["course"], student_id),
        )
        db.commit()
